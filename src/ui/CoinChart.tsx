...
export const CoinChart: React.FC<Props> = ({coinId, loadedStatus, coinDeatiledData, themeStatus}) => {
  ...
  const fetchingHistoricalData = useCallback(async () => {
    setloadStatus(false);
    let url = HistoricalChart(coinId, days, "usd");
    ...
    setloadStatus(true);
  }, [coinId, days, dataName]);

  useEffect(() => {
    fetchingHistoricalData();
  }, [fetchingHistoricalData]);

  const onSetdataHandler: any = (e: { target: { value: React.SetStateAction<string>; }; }) => {
    let day = +e.target.value;
    setDays(day);
  };

  const onSetDataNameHandler: any = (e: { target: { value: React.SetStateAction<string>; }; }) => {
      ...
      <div>
        {loadedStatus && (
          <p className="w-full text-xs text-grey-600 text-left">
            {coinDeatiledData.name} Price
          </p>
        )}
        {!loadedStatus && <Skeleton className="w-full h-16 mb-2" />}
        {loadedStatus && (
          <div className="py-2 flex justify-between items-center">
            <h1 className="text-3xl font-bold">
              ${coinDeatiledData.price}
            </h1>
            <span
              className={`text-sm py-1 px-2 h-fit text-white rounded-md ${
                coinDeatiledData.prcpertf > 0
                  ? "bg-light-green-500"
                  : "bg-red-500"
              }`}
            >
              ...
              {coinDeatiledData.prcpertf}%
            </span>
          </div>
        )}
        {!loadedStatus && <Skeleton className="w-full h-6 mb-2" />}
        {loadedStatus && (
          <div className="py-2 flex justify-between items-center">
            <p className="text-xs text-grey-500">
              ...
              <span
                className={`font-bold ${
                  themeStatus === true ? "text-black" : "text-white"
                }`}
              >
                ${coinDeatiledData.lowtfprc}
              </span>
            </p>
            <Tags className="text-xs text-grey-500" >24h</Tags>
            <p className="text-xs text-grey-500">
              High:
              <span
                className={`font-bold ${
              ...
              >
                ${coinDeatiledData.hightfprc}
              </span>
            </p>
          </div>
        )}
        {!loadedStatus && <Skeleton className="w-full h-28 mb-2" />}
        {loadedStatus && (
          <div className="mb-6">
            ...
            <div className="py-1 flex justify-between items-center">
              ...
              <span className="font-medium">
                ${coinDeatiledData.circulating_supply}
              </span>
            </div>
          </div>
        )}
